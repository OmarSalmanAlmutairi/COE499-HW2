import base64

class AESKeyExpansion:
   # AES constants for key expansion
    RCON = [
        0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A
    ]

    # AES S-Box for the key expansion subroutine
    S_BOX = [
        
    ]
    def __init__(self, base64_key):
        # Initialize with a base64-encoded key (AES-128 or AES-256)
        self.key = self.decode_base64_key(base64_key)
        self.key_size = len(self.key) * 8  # Size in bits (128 or 256)
        self.n_rounds = self.get_number_of_rounds()
        self.round_keys = self.key_expansion()

    @staticmethod
    def decode_base64_key(base64_key):
        """
        Decodes the Base64-encoded key and returns the binary key.
        Supports AES-128 (16 bytes) and AES-256 (32 bytes).
        """
        decoded_key = base64.b64decode(base64_key)
        key_length = len(decoded_key)
        if key_length not in [16, 32]:
            raise ValueError("Key must be 128-bit (16 bytes) or 256-bit (32 bytes).")
        return decoded_key

    def get_number_of_rounds(self):
        """
        Determines the number of rounds based on the key size.
        AES-128 uses 10 rounds, AES-256 uses 14 rounds.
        """
        if self.key_size == 128:
            return 10
        elif self.key_size == 256:
            return 14
        else:
            raise ValueError("Unsupported key size. Key must be either 128-bit or 256-bit.")

    @staticmethod
    def rotate(word):
        """
        Rotates a word (4-byte list) by shifting left (circular shift).
        """
        return word[1:] + word[:1]

    def sub_word(self, word):
        """
        Substitutes each byte in the word using the S-box.
        """
        return [self.S_BOX[b] for b in word]

    def key_expansion(self):
        """
        Expands the original key into round keys.
        """
        expanded_keys = list(self.key)  # Start with the original key as a list of bytes
        key_size = len(self.key)  # In bytes (16 for AES-128, 32 for AES-256)

        # Generate the expanded keys (n_rounds + 1 round keys, each 16 bytes)
        for i in range(key_size, (self.n_rounds + 1) * 16, 4):
            temp = expanded_keys[i-4:i]
            
            # Apply the key expansion core operation at regular intervals
            if i % key_size == 0:
                temp = self.sub_word(self.rotate(temp))
                temp[0] ^= self.RCON[i // key_size - 1]
            elif key_size == 32 and i % key_size == 16:
                temp = self.sub_word(temp)

            # XOR temp with the word key_size bytes before
            expanded_keys += [x ^ y for x, y in zip(temp, expanded_keys[i-key_size:i-key_size+4])]

        return self.get_round_keys(expanded_keys)

    def get_round_keys(self, expanded_keys):
        """
        Organizes the expanded keys into round keys (16-byte chunks).
        """
        round_keys = []
        for i in range(self.n_rounds + 1):
            round_key = expanded_keys[i*16:(i+1)*16]
            round_keys.append(round_key)
        return round_keys

    def write_keys_to_file(self, filename='aes_keys.txt'):
        """
        Writes the AES key and round keys to a file in hexadecimal format.
        """
        with open(filename, 'w') as f:
            f.write(f"AES Key: {''.join([f'{byte:02x}' for byte in self.key])}\n")
            f.write("AES Round Keys:\n")
            for i, round_key in enumerate(self.round_keys):
                f.write(f"Round {i}: {''.join([f'{byte:02x}' for byte in round_key])}\n")

# Example usage
def main():
    # Base64 encoded AES-128 or AES-256 key
    base64_key = "sI7p6arwdHc0CeVnr48gxMOIiK5HkUNMUz2TT6tR++k="  # AES-256 example
    
    # Initialize the AESKeyExpansion class
    aes_expansion = AESKeyExpansion(base64_key)

    # Write the expanded keys to a file
    aes_expansion.write_keys_to_file()

    print(f"AES Key and round keys have been generated and saved to aes_keys.txt.")

if __name__ == "__main__":
    main()
